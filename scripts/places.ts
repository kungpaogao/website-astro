...
async function getData() {
  ...
  console.log("getData:", "Launching browser...");
  ...
  try {
    ...
    console.log("getData:", `Found ${parsedData.length} places`);

    // Save parsed data
    await fs.writeFile(PLACES_SAVE_PATH, JSON.stringify(parsedData, null, 2));
    console.log("getData:", `Saved ${parsedData.length} places to ${PLACES_SAVE_PATH}`);
  } catch (error) {
